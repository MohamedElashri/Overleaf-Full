...
module.exports = LatexRunner = {
  runLatex(project_id, options, callback) {
    ...
    if (mainFile.match(/\.md$/)) {
        command = LatexRunner._pandocCommand(directory, mainFile, flags)
    } else {
  ...
  _pandocCommand(directory, mainFile, flags) {
    let command = [
      'pandoc.wrapper',
    ...
    if (fs.existsSync(Path.join(directory, 'config.yaml'))) {
      command.push('-dconfig')
    }
    return command;
  },
